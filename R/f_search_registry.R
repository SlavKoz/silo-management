...
f_get_search_items <- function(category = "all", query = "", pool = NULL, limit = 50) {
  items <- list()
  cat(sprintf("[SearchRegistry] category=%s query=\"%s\" len=%d pool=%s limit=%d\n",
              category, query, nchar(query), if (is.null(pool)) "NULL" else "OK", limit))
  flush.console()
  ...
  if (category == "all") {
    if (!is.null(pool)) {
      ...
      for (cat in all_categories) {
        ...
        # Items keep their original labels - grouping will be shown via optgroup
    ...
    tryCatch({
      if (category == "containers") {
        ...
        if (nzchar(query)) {
          ...
          sql <- sprintf(
            "SELECT TOP %d ContainerTypeID, TypeName FROM SiloOps.dbo.ContainerTypes WHERE TypeName LIKE '%%%s%%'",
            limit, safe_query
          )
        } else {
          sql <- sprintf(
            "SELECT TOP %d ContainerTypeID, TypeName FROM SiloOps.dbo.ContainerTypes",
            limit
          )
        }
        ...
        if (nrow(df) > 0) {
          items <- lapply(1:nrow(df), function(i) {
            list(
              id = df$ContainerTypeID[i],
              label = df$TypeName[i],
              category = "containers",
              route = paste0("#/containers/", df$ContainerTypeID[i])
            )
          })
        }
      }
  ...
  cat(sprintf("[SearchRegistry] returning %d items for category=%s\n", length(items), category))
  flush.console()
  items
}

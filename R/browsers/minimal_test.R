# Minimal test module - just ONE dropdown
# Purpose: Isolate why dropdowns don't populate in app but work in test
...
minimal_test_server <- function(id, pool, route = NULL) {
  moduleServer(id, function(input, output, session) {
    cat("\n========================================\n")
    cat("[MINIMAL TEST] Module server started\n")
    cat("========================================\n")
    ...
    layouts_data <- reactive({
      ...
      cat("[MINIMAL TEST] layouts_data reactive called\n")
      df <- try(list_canvas_layouts(limit = 100), silent = TRUE)

      if (inherits(df, "try-error") || is.null(df)) {
        cat("[MINIMAL TEST] ERROR: Query failed\n")
        return(data.frame())
      }

      cat("[MINIMAL TEST] Query returned", nrow(df), "rows\n")
      return(df)
    })

    # Watch for route changes to "minimal" (same pattern as placements)
    if (!is.null(route) && is.function(route)) {
      observe({
        current_route <- route()
        cat("[MINIMAL TEST] Route changed to:", paste(current_route, collapse="/"), "\n")

        if (length(current_route) > 0 && current_route[1] == "minimal") {
          if (!ui_initialized()) {
            cat("[MINIMAL TEST] First navigation to minimal, triggering refresh\n")
            ui_initialized(TRUE)
    ...
    observe({
      cat("[MINIMAL TEST] Populating layout dropdown observer fired\n")
      layouts <- layouts_data()
      cat("[MINIMAL TEST] Found", nrow(layouts), "layouts\n")

      if (nrow(layouts) > 0) {
        choices <- setNames(layouts$LayoutID, layouts$LayoutName)
        cat("[MINIMAL TEST] Calling updateSelectInput with", length(choices), "choices\n")
        cat("[MINIMAL TEST] Choices:", paste(names(choices), collapse=", "), "\n")

        updateSelectInput(session, "test_dropdown", choices = choices, selected = choices[1])

        cat("[MINIMAL TEST] updateSelectInput called\n")
      } else {
        cat("[MINIMAL TEST] No layouts found\n")
    ...
    cat("[MINIMAL TEST] Module server initialization complete\n")
  })
}

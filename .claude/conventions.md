...
- `.claude/table_structures.csv` - Complete database schema (all tables, columns, data types)
- `.claude/table_constraints.csv` - Database constraints (PRIMARY KEY, FOREIGN KEY, UNIQUE, CHECK)
- `.claude/conventions.md` - This file (project conventions)

Keep record on what we are doing in the session_summary.md

**Database Schema Files:**
- Both CSV files are generated by `getSQLTables.R` script
- Run the script to refresh schema information after database changes
- Files are located in `.claude/` folder for easy access

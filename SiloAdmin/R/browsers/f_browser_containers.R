      ...
      on_save = function(data) {
        tryCatch({
          ...
          if (grepl("UNIQUE KEY constraint", error_msg, ignore.case = TRUE)) {
            # Duplicate key - likely the Name field
            field_to_clear <- "TypeName"
            if (grepl("duplicate key value is \\(([^)]+)\\)", error_msg, ignore.case = TRUE)) {
              ...
              user_msg <- paste0("Cannot save: Name '", dup_value, "' already exists. Please use a different name.")
            } else {
              user_msg <- "Cannot save: This name already exists. Please use a different name."
            }
    ...
    if (!is.null(route) && shiny::is.reactive(route)) {
      observeEvent(route(), {
        ...
        if (length(parts) >= 1 && parts[1] == "containers") {
          ...
          if (length(parts) >= 2) {
            container_type_id <- suppressWarnings(as.integer(parts[2]))
            if (is.na(container_type_id)) return()

            df <- raw_types()
            if (!nrow(df)) return()

            row <- df[df$ContainerTypeID == container_type_id, ]
            if (nrow(row) == 0) {
              showNotification(paste0("Container type '", container_type_id, "' not found"), type = "warning", duration = 2)
              return()
            }
      ...
      observe({
        ...
        expected_parts <- c("containers", as.character(sid))

        if (!identical(parts, expected_parts)) {
          # Send message to update hash
          session$sendCustomMessage("set-hash", list(h = paste0("#/containers/", sid)))
        }
      })
    }

...
.ALLOWED_CONTAINER_TYPES_COLS <- c(
  "ContainerTypeID","TypeName","Description",
...
update_canvas_area <- function(canvas_id, area_id = NULL) {
  db_execute_params("
    ...
    WHERE id = ?
  ", list(
    if (is.null(area_id) || area_id == "") NA_integer_ else as.integer(area_id),
...
update_layout_background <- function(layout_id, canvas_id = NULL, rotation = NULL,
  ...
  if (!is.null(canvas_id)) {
    ...
    params <- c(params, list(if (is.null(canvas_id) || is.na(canvas_id) || canvas_id == "") NA_integer_ else as.integer(canvas_id)))
  }
  if (!is.null(rotation)) {
  ...
  if (!is.null(site_id)) {
    ...
    params <- c(params, list(if (is.null(site_id) || is.na(site_id) || site_id == "") NA_integer_ else as.integer(site_id)))
  }
...
list_container_types <- function(name_like = NULL,
                                 order_col = "TypeName",
                                 order_dir = "ASC",
                                 limit = 500,
                                 offset = 0) {
  ...
  where <- c(); params <- list()
  if (!is.null(name_like)) { where <- c(where, "TypeName LIKE ?"); params <- c(params, list(name_like)) }
  where_sql <- if (length(where)) paste("WHERE", paste(where, collapse = " AND ")) else ""

  sql <- sprintf("
    SELECT
      ct.ContainerTypeID, ct.TypeName, ct.Description,
...
upsert_container_type <- function(data) {
  ...
  type_name <- f_or(data$TypeName, "")
  if (!nzchar(type_name)) stop("TypeName is required")
